...
def build(semantic_dir, output):
    ...
    for timing in schedule.itertuples(index=False):
        ...
        market=build_market_modules(cutoff,tickers,bounded_calendar,bounded_prices,indices.loc[indices.trade_date.le(end)])
        ek9values={t:m['Ek9']['value'] for t,m in market['per_ticker'].items() if m['Ek9']['value'] is not None}
        core=build_core_modules(semantics,cutoff,tickers)
        ...
        for ticker in tickers:
            ...
            if family is None: reasons.append('HISTORICAL_SECTOR_FAMILY_EVIDENCE_MISSING')
            ...
            cell['partial_core_diagnostic_reasons']=core['per_ticker'][ticker]['reasons']
            ...
            context=CompanyModuleContext(ticker,components,None,True,'CORE_VAL_CONTEXT_INCOMPLETE',end.date(),cutoff)
            ...
            scored={'contract':'EXPERIMENTAL_P4_CELL_V1','profile':PROFILE,'month':str(timing.month),'signal_date':signal,'knowledge_cutoff_at':cutoff.isoformat(),'ticker':ticker,'status':'EXPLICIT_REJECTION','rank':None,'decision':result.decision,'final_score':result.final_score,'base_score':result.base_score,'veto':result.veto_flag,'module_values':module_values,'missing_modules':list(result.missing_modules),'reasons':reasons,'combiner_status':result.total_rasyo_status,'p3_cell_sha256':hashlib.sha256(encoded(cell)).hexdigest()}
