    ...
    for timing in schedule.itertuples(index=False):
        ...
        for ticker in tickers:
            ...
            if not current.empty:
                p=current.iloc[-1]
                price={'trade_date':p.trade_date.date().isoformat(),'close':float(p.close),
                    'adj_close':float(p.adj_close),'yahoo_symbol':str(p.yahoo_symbol),
                    'source_ticker':str(p.price_source_ticker),'resolution':str(p.price_resolution),
                    'file_sha256':price_sha,'valuation_basis_verified':False}
