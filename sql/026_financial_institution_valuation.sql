...
-- deger leme tablolari.
--
-- Alt gruplar AYRI emsal havuzlaridir; business_type CHECK ile kisitlanir.
-- Metrik anlik goruntuleri DEGISTIRILEMEZ (immutable trigger).
-- ============================================================================

CREATE TABLE IF NOT EXISTS core.financial_institution_metrics_snapshots (
  ...
  business_type TEXT NOT NULL
    CHECK (business_type IN ('FACTORING','LEASING','CONSUMER_FINANCE')),
  ...
  average_equity NUMERIC NOT NULL CHECK (average_equity > 0),
  net_income_ttm NUMERIC NOT NULL,
  total_assets NUMERIC NOT NULL CHECK (total_assets > 0),
  finance_receivables NUMERIC NOT NULL CHECK (finance_receivables >= 0),
  npl_gross NUMERIC CHECK (npl_gross >= 0),
  provisions NUMERIC CHECK (provisions >= 0),
  net_finance_income_ttm NUMERIC,
  funding_cost_ttm NUMERIC CHECK (funding_cost_ttm >= 0),
  operating_expenses_ttm NUMERIC CHECK (operating_expenses_ttm >= 0),
  capital_adequacy_ratio NUMERIC CHECK (capital_adequacy_ratio >= 0),
  ...
  CHECK ((extract(month FROM period_end), extract(day FROM period_end))
         IN ((3,31),(6,30),(9,30),(12,31))),
  CHECK (total_equity <= total_assets),
  CHECK (finance_receivables <= total_assets),
  CHECK (npl_gross IS NULL OR npl_gross <= finance_receivables),
  -- Karsilik varsa takip alacagi da bilinmeli; aksi halde kapsam orani turetilemez.
  CHECK (provisions IS NULL OR npl_gross IS NOT NULL)
);

CREATE INDEX IF NOT EXISTS idx_fi_metrics_pit
  ON core.financial_institution_metrics_snapshots (
...
CREATE OR REPLACE FUNCTION core.guard_fi_metrics_mutation()
RETURNS trigger LANGUAGE plpgsql AS $$
DECLARE
  requested_inserted_at TIMESTAMPTZ;
BEGIN
  IF TG_OP = 'DELETE' THEN
    RAISE EXCEPTION 'financial_institution_metrics_snapshots immutable';
  END IF;
...
DROP TRIGGER IF EXISTS trg_fi_metrics_immutable
  ON core.financial_institution_metrics_snapshots;
CREATE TRIGGER trg_fi_metrics_immutable
BEFORE UPDATE OR DELETE ON core.financial_institution_metrics_snapshots
FOR EACH ROW EXECUTE FUNCTION core.guard_fi_metrics_mutation();

CREATE TABLE IF NOT EXISTS analytics.financial_institution_valuation_periods (
  ticker TEXT NOT NULL,
  ...
  npl_ratio NUMERIC,
  provision_coverage NUMERIC,
  net_finance_margin NUMERIC,
  equity_buffer NUMERIC NOT NULL CHECK (equity_buffer > 0),
...
CREATE INDEX IF NOT EXISTS idx_fi_valuation_latest
  ON analytics.financial_institution_valuation_periods(ticker, analysis_at DESC, period_end DESC);
CREATE INDEX IF NOT EXISTS idx_fi_valuation_group
  ON analytics.financial_institution_valuation_periods(
    business_type, accounting_profile, analysis_at DESC, period_end DESC);

CREATE TABLE IF NOT EXISTS analytics.financial_institution_valuation_rejections (
  ticker TEXT NOT NULL,
  analysis_at TIMESTAMPTZ NOT NULL,
  valuation_profile TEXT NOT NULL,
  valuation_version INT NOT NULL CHECK (valuation_version > 0),
...
CREATE INDEX IF NOT EXISTS idx_fi_rejections_latest
  ON analytics.financial_institution_valuation_rejections(analysis_at DESC, ticker);

CREATE TABLE IF NOT EXISTS analytics.financial_institution_m2_scores (
  ticker TEXT NOT NULL,
  analysis_at TIMESTAMPTZ NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_fi_m2_latest
  ON analytics.financial_institution_m2_scores(ticker, analysis_at DESC, period_end DESC);

CREATE OR REPLACE VIEW analytics.latest_financial_institution_m2_scores AS
