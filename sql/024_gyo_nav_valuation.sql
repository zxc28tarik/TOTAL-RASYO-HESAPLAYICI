CREATE TABLE IF NOT EXISTS core.gyo_nav_snapshots (
  ...
  property_portfolio_value NUMERIC NOT NULL CHECK (property_portfolio_value > 0),
  cash_and_financial_assets NUMERIC CHECK (cash_and_financial_assets >= 0),
  other_assets NUMERIC CHECK (other_assets >= 0),
  total_liabilities NUMERIC CHECK (total_liabilities >= 0),
  nav_source_method TEXT NOT NULL CHECK (nav_source_method IN ('DIRECT', 'DERIVED')),
  ...
  PRIMARY KEY (ticker, nav_asof_date, published_at, nav_profile, nav_version, source_document_id),
  CHECK (nav_asof_date <= (published_at AT TIME ZONE 'Europe/Istanbul')::date),
  CHECK (
    nav_source_method = 'DIRECT'
    OR (cash_and_financial_assets IS NOT NULL AND other_assets IS NOT NULL AND total_liabilities IS NOT NULL)
  )
);

CREATE INDEX IF NOT EXISTS idx_gyo_nav_pit
  ON core.gyo_nav_snapshots (
    ticker, nav_profile, nav_version, nav_asof_date DESC, published_at DESC,
    version_sequence DESC, source_document_id DESC, source_sha256 DESC
  );

CREATE OR REPLACE FUNCTION core.reject_gyo_nav_mutation()
RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
  IF TG_OP = 'DELETE' THEN
    RAISE EXCEPTION 'gyo_nav_snapshots immutable';
  END IF;
...
DROP TRIGGER IF EXISTS trg_gyo_nav_immutable ON core.gyo_nav_snapshots;
CREATE TRIGGER trg_gyo_nav_immutable
BEFORE UPDATE OR DELETE ON core.gyo_nav_snapshots
FOR EACH ROW EXECUTE FUNCTION core.reject_gyo_nav_mutation();

CREATE TABLE IF NOT EXISTS analytics.gyo_valuation_periods (
  ticker TEXT NOT NULL,
  ...
  v_mid NUMERIC,
  v_high NUMERIC,
  target_pd_nav NUMERIC,
  current_pd_nav NUMERIC NOT NULL CHECK (current_pd_nav > 0),
...
CREATE INDEX IF NOT EXISTS idx_gyo_valuation_latest
  ON analytics.gyo_valuation_periods(ticker, analysis_at DESC, nav_asof_date DESC);
CREATE INDEX IF NOT EXISTS idx_gyo_valuation_group
  ON analytics.gyo_valuation_periods(peer_group, analysis_at DESC, nav_asof_date DESC);

CREATE TABLE IF NOT EXISTS analytics.gyo_valuation_rejections (
  ticker TEXT NOT NULL,
  analysis_at TIMESTAMPTZ NOT NULL,
  valuation_profile TEXT NOT NULL,
  valuation_version INT NOT NULL CHECK (valuation_version > 0),
...
CREATE INDEX IF NOT EXISTS idx_gyo_rejections_latest
  ON analytics.gyo_valuation_rejections(analysis_at DESC, ticker);

CREATE TABLE IF NOT EXISTS analytics.gyo_m2_scores (
  ticker TEXT NOT NULL,
  analysis_at TIMESTAMPTZ NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_gyo_m2_latest
  ON analytics.gyo_m2_scores(ticker, analysis_at DESC, nav_asof_date DESC);

CREATE OR REPLACE VIEW analytics.latest_gyo_m2_scores AS
