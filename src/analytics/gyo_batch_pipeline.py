...
class GyoBatchError(ValueError):
    pass


def _aware_datetime(name: str, value: Any) -> datetime:
    if not isinstance(value, datetime) or value.tzinfo is None or value.utcoffset() is None:
        raise GyoBatchError(f"{name} timezone iceren datetime olmali")
    return value
...
def _coerce_aware_datetime(name: str, value: Any) -> datetime:
    if isinstance(value, pd.Timestamp):
        ...
        try:
            ...
            raise GyoBatchError(f"{name} ISO datetime olmali") from exc
...
def _normalize_tickers(values: Optional[Iterable[str]]) -> Optional[list[str]]:
    ...
    for value in values:
        if not isinstance(value, str) or not value.strip():
            raise GyoBatchError("ticker degerleri dolu metin olmali")
...
def fetch_gyo_universe(
    ...
    *,
    ...
    for row in df.itertuples(index=False):
        ...
        if ticker in seen:
            raise GyoBatchError(f"evren sorgusu yinelenen ticker dondurdu: {ticker}")
        seen.add(ticker)
        family = config.route(
            ticker=ticker,
    ...
    *,
    ...
    if not ticker_list:
        return pd.DataFrame(columns=[
            ...
            "property_portfolio_value", "nav_source_method", "source_confidence", "source_document_id", "source_sha256", "nav_profile", "nav_version",
        ])
    return pd.read_sql(
        """
        SELECT DISTINCT ON (ticker)
               ticker, nav_asof_date, published_at AS nav_published_at,
               nav_total, shares_out, share_basis, currency, property_portfolio_value, nav_source_method, source_confidence,
               source_document_id, source_sha256, nav_profile, nav_version
        FROM core.gyo_nav_snapshots
...
def build_gyo_snapshots_from_frames(
    *,
...
) -> tuple[list[GyoSnapshot], list[dict[str, str]]]:
    analysis = _aware_datetime("analysis_at", analysis_at)
    ...
    for name, frame, required in (
        ...
        if not isinstance(frame, pd.DataFrame):
            raise GyoBatchError(f"{name} DataFrame olmali")
        missing = required - set(frame.columns)
        if missing:
            raise GyoBatchError(f"{name} eksik alanlar: {', '.join(sorted(missing))}")

    universe_rows: dict[str, Mapping[str, Any]] = {}
    for row in universe.to_dict("records"):
        ticker = str(row["ticker"]).strip().upper()
        if ticker in universe_rows:
            raise GyoBatchError(f"universe yinelenen ticker: {ticker}")
        if str(row["sector_family"]).strip().upper() != "GYO":
            raise GyoBatchError(f"GYO batch beklenmeyen aile: {ticker}")
    ...
    def unique_map(name: str, frame: pd.DataFrame) -> dict[str, Mapping[str, Any]]:
        ...
        for row in frame.to_dict("records"):
            ticker = str(row["ticker"]).strip().upper()
            if ticker in result:
                raise GyoBatchError(f"{name} yinelenen ticker: {ticker}")
            result[ticker] = row
        return result

    nav_rows = unique_map("navs", navs)
    price_rows = unique_map("prices", prices)
    snapshots: list[GyoSnapshot] = []
    ...
    for ticker in sorted(universe_rows):
        ...
        try:
            ...
            snapshots.append(build_gyo_snapshot(
                ticker=ticker,
                analysis_at=analysis,
                ...
                property_portfolio_value=nav["property_portfolio_value"],
                nav_source_method=nav["nav_source_method"],
        ...
        except (GyoValuationError, TypeError, ValueError, OverflowError) as exc:
...
def _follow_context_map(frame: pd.DataFrame) -> dict[str, dict[str, Any]]:
    ...
    missing = required - set(frame.columns)
    if missing:
        raise GyoBatchError("follow frame eksik alanlar: " + ", ".join(sorted(missing)))
    result: dict[str, dict[str, Any]] = {}
    for row in frame.to_dict("records"):
        ticker = str(row["ticker"]).strip().upper()
        if ticker in result:
            raise GyoBatchError(f"follow frame yinelenen ticker: {ticker}")
        if type(row["follow_active"]) is not bool:
            raise GyoBatchError("follow_active Python bool olmali")
...
def _strict_text(name: str, value: Any, *, uppercase: bool = False) -> str:
    if not isinstance(value, str) or not value.strip():
        raise GyoBatchError(f"{name} dolu metin olmali")
    result = value.strip()
...
def _strict_positive_int(name: str, value: Any) -> int:
    if type(value) is not int or value <= 0:
        raise GyoBatchError(f"{name} pozitif Python int olmali")
    return value
...
def _strict_sha256(name: str, value: Any) -> str:
    ...
    if len(text) != 64 or any(ch not in "0123456789abcdef" for ch in text):
        raise GyoBatchError(f"{name} 64 karakter hex SHA256 olmali")
    return text
...
def _finite_number(
    ...
    if isinstance(value, bool):
        raise GyoBatchError(f"{name} bool olamaz")
    try:
        result = float(value)
    except (TypeError, ValueError, OverflowError) as exc:
        raise GyoBatchError(f"{name} sayisal olmali") from exc
    if not math.isfinite(result):
        raise GyoBatchError(f"{name} sonlu olmali")
    if minimum is not None and (result <= minimum if strict_minimum else result < minimum):
        ...
        raise GyoBatchError(f"{name} {minimum} {relation}")
    if maximum is not None and result > maximum:
        raise GyoBatchError(f"{name} {maximum} degerini asamaz")
    return result


def persist_gyo_batch(conn: Any, report: Mapping[str, Any]) -> None:
    if not isinstance(report, Mapping):
        raise GyoBatchError("report mapping olmali")
    results = report.get("results")
    if not isinstance(results, list):
        raise GyoBatchError("report.results liste olmali")
    result_count = report.get("result_count")
    if type(result_count) is not int or result_count != len(results):
        raise GyoBatchError("report.result_count results uzunluguyla eslesmeli")

    rejection_rows = report.get("rejections", [])
    if not isinstance(rejection_rows, list):
        raise GyoBatchError("report.rejections liste olmali")
    ...
    for index, row in enumerate(results):
        if not isinstance(row, Mapping):
            raise GyoBatchError(f"results[{index}] mapping olmali")
        ...
        if not isinstance(valuation, Mapping) or not isinstance(m2, Mapping):
            raise GyoBatchError("report valuation/m2 mapping olmali")
        _require_keys(f"results[{index}].valuation", valuation, {
            ...
            "current_price", "nav_published_at", "nav_total", "shares_out", "property_portfolio_value", "nav_per_share",
            "nav_source_method", "source_confidence", "source_document_id", "source_sha256", "nav_profile", "nav_version",
            "valuation_profile", "valuation_version", "config_sha256", "status",
            "valuation_score", "v_conf", "current_pd_nav",
        })
        ...
        if row_ticker != valuation_ticker or valuation_ticker != m2_ticker:
            raise GyoBatchError("result, valuation ve m2 ticker uyusmuyor")
        if valuation_ticker in success_seen:
            raise GyoBatchError(f"yinelenen result ticker: {valuation_ticker}")
        ...
        if valuation_analysis != analysis_at or m2_analysis != analysis_at:
            raise GyoBatchError("valuation/m2 analysis_at report.analysis_at ile uyusmuyor")
        if valuation["nav_asof_date"] != m2["nav_asof_date"]:
            raise GyoBatchError("valuation ve m2 nav_asof_date uyusmuyor")
        if not isinstance(valuation["nav_asof_date"], date) or isinstance(valuation["nav_asof_date"], datetime):
            raise GyoBatchError("valuation.nav_asof_date date olmali")
        if not isinstance(valuation["price_trade_date"], date) or isinstance(valuation["price_trade_date"], datetime):
            raise GyoBatchError("valuation.price_trade_date date olmali")
        ...
        _finite_number("valuation.property_portfolio_value", valuation["property_portfolio_value"], minimum=0.0, strict_minimum=True)
        _strict_text("valuation.nav_source_method", valuation["nav_source_method"], uppercase=True)
        ...
        _finite_number("valuation.current_pd_nav", valuation["current_pd_nav"])
        _finite_number("m2.m2", m2["m2"], minimum=0.0, maximum=1.0)
        if type(m2["valuation_usable"]) is not bool:
            raise GyoBatchError("m2.valuation_usable Python bool olmali")

        diagnostics = valuation.get("diagnostics") or {}
        score_inputs = m2.get("score_inputs") or {}
        if not isinstance(diagnostics, Mapping):
            raise GyoBatchError("valuation.diagnostics mapping olmali")
        if not isinstance(score_inputs, Mapping) or not score_inputs:
            raise GyoBatchError("m2.score_inputs dolu mapping olmali")
        try:
            ...
            raise GyoBatchError("gyo diagnostics/score_inputs kanonik JSON olmali") from exc
        ...
        valuation_rows.append((
            ...
            valuation["property_portfolio_value"], _strict_text("valuation.nav_source_method", valuation["nav_source_method"], uppercase=True),
            ...
            valuation.get("target_pd_nav"), valuation.get("current_pd_nav"),
    ...
    for index, rejection in enumerate(rejection_rows):
        if not isinstance(rejection, Mapping):
            raise GyoBatchError(f"rejections[{index}] mapping olmali")
        ...
        if ticker in seen_rejections:
            raise GyoBatchError(f"yinelenen rejection ticker: {ticker}")
    ...
    with conn:
        with conn.cursor() as cur:
            if attempted_tickers:
                ...
                cur.execute(
                    """
                    DELETE FROM analytics.gyo_valuation_periods
                    WHERE analysis_at = %s
                ...
                cur.execute(
                    """
                    DELETE FROM analytics.gyo_m2_scores
                    WHERE analysis_at = %s
                      AND ticker = ANY(%s::text[])
                    """,
            ...
            if success_tickers:
                cur.execute(
                    """
                    DELETE FROM analytics.gyo_valuation_rejections
                    WHERE analysis_at = %s
                      AND valuation_profile = %s
                      AND valuation_version = %s
                      AND ticker = ANY(%s::text[])
            ...
            for ticker, reason in normalized_rejections:
                cur.execute(
                    """
                    INSERT INTO analytics.gyo_valuation_rejections (
                    ...
                    DO UPDATE SET
                      ...
                      attempts = analytics.gyo_valuation_rejections.attempts + 1,
...
def run_gyo_batch(
    ...
    *,
    ...
    if type(persist) is not bool:
        raise GyoBatchError("persist Python bool olmali")
    analysis = _aware_datetime("analysis_at", analysis_at)
    config = validate_gyo_config(config)
    universe = fetch_gyo_universe(conn, tickers=tickers, routing_config=routing_config)
    ticker_list = universe["ticker"].astype(str).tolist() if not universe.empty else []
    navs = fetch_gyo_navs_asof(conn, tickers=ticker_list, analysis_at=analysis, config=config)
    prices = fetch_gyo_prices(conn, tickers=ticker_list, analysis_at=analysis)
    prices = attach_action_bundles(prices, action_bundles)
    follow = fetch_gyo_follow_contexts(conn, tickers=ticker_list, analysis_at=analysis)
    basis_receipts = {}
    snapshots, rejections = build_gyo_snapshots_from_frames(
